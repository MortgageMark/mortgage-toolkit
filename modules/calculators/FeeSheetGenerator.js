...
function FeeSheetGenerator({ isInternal = false, user = null }) {
  ...
  const [ovCondoReview,    setOvCondoReview]    = useLocalStorage("fs_ov_condo",  "");
  ...
  const fees = useMemo(() => {
    ...
    const condoReviewFee = pcPropType === "condo" ? ov(ovCondoReview, 450) : 0;
    const thirdPartyFees = appraisal + appraisalReview + creditReport + floodCert + taxService + docPrep + survey + t47Fee + pestInspection + dpaBondFee + dpaProgFees + dpaOptFees + condoReviewFee;
    ...
    return {
      ...
      thirdPartyFees, appraisal, appraisalReview, creditReport, floodCert, taxService, survey, docPrep, pestInspection, condoReviewFee, dpaBondFee,
      ...
      s2LienEnabled,
      ovCreditReport, ovFloodCert, ovTaxService, ovSurvey, ovHomeWarranty, ovPestInspection, ovBondFee, ovCondoReview,
      ...
      fsTempBd, term, pcUpfrontMode, pcPropType]);
  ...
  async function downloadFeeSheetPDF() {
    ...
    try {
      ...
      // Small "Generated by" footer
      const generatorName = (user && (user.display_name || user.name)) ? (user.display_name || user.name) : null;
      if (generatorName) {
        doc.setFontSize(7);
        doc.setTextColor(160, 160, 160);
        doc.text("Generated by " + generatorName, PW / 2, PH - 6, { align: "center" });
      }
      const d = new Date();
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <SectionCard title="CLOSING & FUNDING DATES" accent={COLORS.blue} infoTip="The closing date is when the title transfers and documents are signed. The funding date (typically 1-3 days later for purchases) is when the lender wires the money and the deal is complete. For refinances in Texas, there's a mandatory 3-day rescission period after signing before funding can occur.">
            <div style={{ marginBottom:12 }}>
              ...
              {!closingDate && (
                <div style={{ marginTop:4, fontSize:12, color:"#0369a1", fontFamily:font, fontStyle:"italic" }}>
                  Using estimated date ({(() => { const p = defaultCD.split("-"); return (parseInt(p[1]))+"/"+parseInt(p[2])+"/"+p[0]; })()} ) for per diem interest. Enter a closing date to get an exact figure.
                </div>
              )}
              {fees.shortPayAdj > 0 ? (
        ...
        <div>
          ...
          <SectionCard title={`CLOSING COST ESTIMATE — ${stateName.toUpperCase()}`}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              {fees.condoReviewFee > 0 && <FeeRow label="Condo Review Fee" amount={fees.condoReviewFee} indent editKey="condo" editValue={ovCondoReview} onEdit={setOvCondoReview} defaultVal={450} isInternal={canEditFees} />}
          ...
          {/* PDF Download button — internal users only */}
          {isInternal && (
      ...
      <div ref={pdfRef} style={{ position:"fixed", left:-9999, top:0, width:816, background:"#fff", fontFamily:"'Segoe UI',Arial,sans-serif" }}>
        ...
        <div style={{ padding:"7px 24px 6px", display: pdfDualCol ? "flex" : "block", gap:20, alignItems:"flex-start" }}>
          ...
          <div style={{ flex: pdfDualCol ? "1 1 0%" : undefined, minWidth:0 }}>
            ...
            {fees.condoReviewFee > 0    && <PdfRow label="Condo Review Fee"          amt={fees.condoReviewFee}  indent />}
